...
public class StrikePoint : MonoBehaviour, IMiniGame
{
    ...
    private MiniGameResultPresenter resultPresenter;
    ...
    private void Awake()
    {
        ...
        resultPresenter = GetComponent<MiniGameResultPresenter>();
    }
    ...
    public async UniTask<MiniGameResult> Play()
    {
        ...
        // 결과 산출 (시간 초과시 미스, 클릭시 판정)
        MiniGameResult result = !isClicked
            ? new MiniGameResult() { grade = MiniGameGrade.Miss }
            : EvaluateResult(hitPoint);

        // 결과 연출 재생 (끝날 때까지 대기)
        await resultPresenter.ShowResult(result.grade);
